...
print ('computing Linear and RBF Kernel', end='\n')
...
for lam in [0, 0.01, 0.1, 0.2, 0.9, 1]:	# possible lambda values for the EasyMKL algorithm
    ...
    acc = np.mean(scores)
    if not best_results_lam or best_results_lam['score'] < acc:
        best_results_lam = {'lam' : lam, 'score' : acc}
print('Best validation accuracy: %.3f with lambda: %i' %(best_results_lam['score'],best_results_lam['lam']))
...
clf = EasyMKL(learner=SVC(C=best_results['C']), lam=best_results_lam['lam']).fit(KLtr,Ytr)
...
f= open("Figs/BreastPancreas.txt","w+")
