...
history = classifier.fit(X_train,y_train, validation_split=.2, batch_size=1, epochs=15, verbose=0)
...
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.savefig('AcccColoRectal.pdf', format='pdf')
...
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper left')
plt.savefig('LossColoRectal.pdf', format='pdf')
plt.show()
